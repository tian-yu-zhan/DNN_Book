...
adaptive.bin.data.func = 
  function(rate.pbo.in, rate.trt.in){
    ...
    if (adap.ind){
      rand.2.pbo = rbinom(n.stage.2.2*ratio.stage.2, 
                          1, rate.pbo.in)
...
DNN.first.input = 
  foreach(first.ind=1:n.first.itt) %dopar% {
...
data.train =  matrix(unlist(DNN.first.input), 
                     nrow = n.first.itt, ncol = 5, byrow=TRUE)
...
########################################################################
library(xtable)
print(xtable(val.para.grid, digits = c(1, 1, 1, 4, 4)),
