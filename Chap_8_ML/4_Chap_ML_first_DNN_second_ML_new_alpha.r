...
###########################################################################
theta.diff.val.vec = c(rep(2, 8), rep(7, 8))
k.val.vec = rep(c(rep(0.3, 4), rep(0.7, 4)),2)
delta.prop.val.vec = rep(c(0, 0.8, 1, 1.1),4)
n.val.itt = 10^5
...
for (val.ind in c(1:16)){
 ...
 val.para.grid[val.ind, c("SVM_1")] = mean(data.stats.val>=(svm.cutof.val.1+0.19))
 
 svm.cutof.val.2 = predict(svm.fit.2, newdata = data.svm.val)
 val.para.grid[val.ind, c("SVM_2")] = mean(data.stats.val>=(svm.cutof.val.2+0.45))
 
 svm.cutof.val.3 = predict(svm.fit.3, newdata = data.svm.val)
 val.para.grid[val.ind, c("SVM_3")] = mean(data.stats.val>=(svm.cutof.val.3+0.08))
 
 svm.cutof.val.4 = predict(svm.fit.4, newdata = data.svm.val)
 val.para.grid[val.ind, c("SVM_4")] = mean(data.stats.val>=(svm.cutof.val.4+3.45))
 ...
 xgboost.cutoff.val.2 =  predict(xgboost.fit.2, 
                                 as.matrix(data.svm.val[,c("theta", "k")]))
 val.para.grid[val.ind, c("XG_2")] = mean(data.stats.val>=(xgboost.cutoff.val.2+0.23))
 
 xgboost.cutoff.val.3 =  predict(xgboost.fit.3, 
                                 as.matrix(data.svm.val[,c("theta", "k")]))
 val.para.grid[val.ind, c("XG_3")] = mean(data.stats.val>=(xgboost.cutoff.val.3+0.03))
 
 xgboost.cutoff.val.4 =  predict(xgboost.fit.4, 
                                 as.matrix(data.svm.val[,c("theta", "k")]))
 val.para.grid[val.ind, c("XG_4")] = mean(data.stats.val>=(xgboost.cutoff.val.4+0.31))
 
 ## random forest
 rf.cutoff.val.1 = predict(rf.fit.1, newdata = data.svm.val)
 val.para.grid[val.ind, c("RF_1")] = mean(data.stats.val>=(rf.cutoff.val.1+0.04))

 rf.cutoff.val.2 = predict(rf.fit.2, newdata = data.svm.val)
 val.para.grid[val.ind, c("RF_2")] = mean(data.stats.val>=(rf.cutoff.val.2+0.06))
 
 rf.cutoff.val.3 = predict(rf.fit.3, newdata = data.svm.val)
 val.para.grid[val.ind, c("RF_3")] = mean(data.stats.val>=(rf.cutoff.val.3+0.05))
 
 rf.cutoff.val.4 = predict(rf.fit.4, newdata = data.svm.val)
 val.para.grid[val.ind, c("RF_4")] = mean(data.stats.val>=(rf.cutoff.val.4+0.07))
...
latex.out.1 = data.frame(
  "theta" = sprintf("%.0f", val.para.grid$theta),
  "k" = sprintf("%.2f", val.para.grid$k),
  "theta_diff" = sprintf("%.3f", val.para.grid$trt_diff),
  "DNN" = paste0(sprintf("%.1f", val.para.grid$DNN_power*100), "%"),
  "t-test" = paste0(sprintf("%.1f", val.para.grid$t_test*100), "%"),
