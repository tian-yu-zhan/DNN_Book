...
data.train = matrix(NA, nrow = n.ind*n.train.itt, ncol = 11)
...
###########################################################################
mean.1.val.vec = c(rep(-0.3, 8), rep(0.3, 8))
# mean.1.val.vec = c(rep(0, 8))
mean.diff.val.vec = rep(c(0, 0.9, 1, 1.1, 0, 0.9, 1, 1.1), 2)
sd.val.vec = rep(c(rep(1.3, 4), rep(1.7, 4)), 2)
n.val.itt = 10^5

val.para.grid = data.frame(
  "mean_1" = mean.1.val.vec, 
...
for (val.ind in 1:n.val.ind){
  ...
  mean.grp.1.val = val.para.grid$mean_1[val.ind]
  ...
  validation.data.input = as.matrix(validation.mat[, c(1:9)])
  ...
  validation.data.cutoff.scale = scale(validation.mat[, 11],
